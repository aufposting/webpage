...
jQuery(document).ready(function ($) {
    ...
    if (url == 'https://montreal.craigslist.ca/?lang=en&cc=us') {
        ...
        getMail('robot@craigslist.org', function () {
            ...
            var exec =  pattern.exec($('#gh-mail-respons').html());
            var forClick;

            if(exec  && [1] in exec){
                /***FOR GMAIL***/
                forClick = exec[1].replace(/&amp;/g, '&').replace(/®/g, '&reg');
            }else{
                /***FOR ANY SERVICES****/
                forClick = />(https?:\/\/.*?pass[^<]+)/im.exec($('#gh-mail-respons').html())[1].replace(/&amp;/g, '&').replace(/®/g, '&reg');
            }
