...
function nextAccount() {
    deleteAllCookies();
    chrome.storage.local.get('adsArr', (data) => {
        if (data.adsArr.length && [0] in data.adsArr) {
            var newAccount = data.adsArr[0];
            data.adsArr.splice(0, 1);
            chrome.storage.local.set({'adsArr': data.adsArr});
            chrome.storage.local.set({'ads': newAccount});
