...
class SitesController extends Controller
{
    ...
    {
        $find = Sites::findOne(['id' => $id]);

        if ($find->is_active == 1) {
            $update_data = Yii::$app
                ->db
                ->createCommand("UPDATE `sites` SET is_active = '0' WHERE id = $id")
                ->execute();
        } else {
            $update_data = Yii::$app
                ->db
                ->createCommand("UPDATE `sites` SET is_active = '1' WHERE id = $id")
                ->execute();
        }
